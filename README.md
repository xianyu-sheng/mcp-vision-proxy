...
**方式 A：VS Code / Cursor（推荐）**

在 VS Code 项目根目录创建 `.mcp.json`：
...
> 路径请根据实际项目位置修改。保存后**重启 VS Code**，MCP 工具 `mcp__vision-proxy__analyze_local_image` 即自动加载。
...
VS Code 从 v1.85+ 内置 MCP 支持。配置路径：**设置 → Extensions → MCP Servers**。

或直接在 VS Code 全局设置 `settings.json`（`Ctrl+,`）中添加：
...
| 进程 | 命令 | 作用 |
|------|------|------|
| 热键监听 | `python main.py` | 捕获剪贴板图片，注入凭证码 |
| MCP 服务 | `python main.py --mcp` | 响应 AI 的 `analyze_local_image` 调用 |

两者可合并运行（`python main.py` 默认即热键+MCP 双模式），也可以分开在两个终端运行。

> **重要**：VS Code 启动后需确保 MCP 服务进程正在运行，否则工具调用会失败。
...
A: 确保 MCP 服务进程正在运行（`python main.py --mcp` 或双模式的 `python main.py`），且 VS Code 已重新加载 MCP 配置。
...
> Gives text-only LLMs (e.g. DeepSeek) vision capabilities via MCP tools. Paste a screenshot → press `Ctrl+Alt+V` → Claude Code / VS Code automatically calls the vision API.
...
# 3. Register MCP tool (VS Code / Cursor)
...
| `--mcp` | MCP Stdio server only (for Claude Code / VS Code integration) |
